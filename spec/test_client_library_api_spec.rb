...
describe 'API tests for customer app' do
  ...
  it 'Post request with a name: in the request body', :regression, :smoke, severity: :critical do |e|
    e.run_step(name: 'Raise a post request with name in the request body') do
      @api.post('/', { 'name': @test_data.fetch('name') })
    end
    e.run_step(name: 'Expect status of the response to be 200') do
      expect(@api.status).to eq(200)
    end
    e.run_step(name: 'Validate the response structure') do
      validate(@api.body,
    ...
    e.run_step(name: 'Validate the sorting of the customers list in the response') do
      validate_list(@api.body, { "key": 'customers', "unit": 'id', "sort": 'ascending' })
    end
    e.run_step(name: 'Validate the response headers') do
  ...
  it 'Post request without a name: in the request body', severity: :critical do |e|
    e.run_step(name: 'Raise a post request with name in the request body') do
      @api.post('/', {})
    end
    e.run_step(name: 'Expect status of the response to be 200') do
